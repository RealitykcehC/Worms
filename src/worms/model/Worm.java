...
public class Worm {
	...
	private boolean useRifle, useBazooka;
	...
	public Worm(double x, double y, double direction, double radius, String name) throws IllegalArgumentException,
		ArithmeticException {
			if (!isValidX(x) || !isValidY(y) || !isValidRadius(radius) || !isValidName(name))
			...
			this.radius = radius;
			this.name = name;
			this.mass = this.density * ((4.0 / 3.0) * Math.PI * Math.pow(radius, 3));
	...
	public boolean canMove(int nbSteps) {
		return (this.getActionPoints() >= (int) Math.ceil(nbSteps * getTotalStepCost()) && nbSteps >= 0);
	}
	 ...
	 * 			| nbSteps < 0
	 */
	public void move(int nbSteps) throws IllegalArgumentException, ArithmeticException{
		if (nbSteps < 0)
			throw new IllegalArgumentException();
		if (this.canMove(nbSteps)) {
			if (!isValidX(this.getX() + nbSteps * Math.cos(this.getOrientation()) * this.getRadius()) || !isValidY(this.getY() + nbSteps * Math.sin(this.getOrientation()) * this.getRadius()))
				throw new ArithmeticException();
			this.x += nbSteps * Math.cos(this.getOrientation()) * this.getRadius();
			this.y += nbSteps * Math.sin(this.getOrientation()) * this.getRadius();
			this.actionPoints -= (int) Math.ceil(nbSteps * this.getTotalStepCost());
		}
	}
	...
	public boolean canTurn(double angle) {
		return (this.getActionPoints() >= Math.round(60 / (int) Math.round(2 * Math.PI / recalculateAngle(angle))));
	}
	...
	public void turn(double angle) {
		...
		int f;
		this.direction = recalculateAngle(recalculateAngle(angle) + this.getOrientation());
	...
	public double [] getJumpStep(double t) {
		...
		double Yt = this.getY() + ((initVelocityY * t) - (.5 * this.g * Math.pow(t, 2)));
		double [] result = {Xt,Yt};
		return result;
	}
	...
	public double getJumpDistance(){
		return (Math.pow(this.getInitialVelocity(), 2) * Math.sin(2 * this.getOrientation())) / this.g;
	}
			...
			for (int i = 0; i < name.length(); i++) {
				if (!(name.charAt(i) == '\'') && !(name.charAt(i) == '\"') && !(name.charAt(i) == ' ') &&
						!(Character.isUpperCase(name.charAt(i))) &&
							!(Character.isLowerCase(name.charAt(i))) && !(Character.isDigit(name.charAt(i)))){
					return false;
				}
			}
			return true;
	...
	public boolean canHaveAsWeaponUsage() {
		return ((this.useRifle() && !this.useBazooka()) || (!this.useRifle() && this.useBazooka()));
	}
	 ...
	 * 			| (new this).getX() == 0 && (new this).getY() == 0 && (new this).getOrientation() == 0 && !(new this).useRifle && !(new this).useBazooka && 
...
//		this = null;
	...
	private void setMass(double newRadius) throws IllegalArgumentException{
		if (!(isValidMass(this.density * ((4.0 / 3.0) * Math.PI * Math.pow(newRadius, 3)))))
			throw new IllegalArgumentException();
		this.mass = this.density * ((4.0 / 3.0) * Math.PI * Math.pow(newRadius, 3));
	...
	private void setMaxActionPoints(double mass) {
		if (Math.round(mass) >= 0) {
			...
			this.actionPoints = Math.min(this.getMaxActionPoints(), this.getActionPoints());
		}
	}
	
	/**
	 * Function that checks whether or not this worm is currently using a rifle.
	 * 
	 * @return true
	 * 			This worm is currently using a rifle
	 * @return false
	 * 			This worm is currently not using a rifle
	 */
	private boolean useRifle() {
		return useRifle;
	}
	
	/**
	 * Function that checks whether or not this worm is currently using a bazooka.
	 * 
	 * @return true
	 * 			This worm is currently using a bazooka
	 * @return false
	 * 			This worm is currently not using a bazooka
	 */
	private boolean useBazooka() {
		return this.useBazooka;
	}
}
