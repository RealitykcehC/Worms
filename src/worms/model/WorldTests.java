...
public class WorldTests {
	...
	public void testWorld(){
		...
		assertEquals(5.0,world.getWidth(), 0.1);
		assertEquals(5.0,world.getHeight(), 0.1);

	}

	@Test (expected = IllegalArgumentException.class)
	public void testConstructorWorld_IllegalWidth()throws Exception{

		World world = new World(illegalCoordinate,5.0, passable, ran);
		assertFalse(World.isValidWidth(world.getWidth()));
	...
	@Test (expected = IllegalArgumentException.class)
	public void testIsValidWidth_IllegalBoundCase() throws Exception{

		World testWorld = new World(illegalCoordinate,illegalCoordinate*10.0, passable, ran);
		assertFalse(World.isValidWidth(testWorld.getWidth()));
	}
	...
	@Test(expected = IllegalArgumentException.class)
	public void testIsValidHeight_IllegalCoordinateCase()throws Exception{
		
		World world = new World (illegalCoordinate*10.0, illegalCoordinate, passable, ran);
		assertFalse(World.isValidHeight(world.getHeight()));
	}
	...
	public void testAddEmptyTeam_IllegalTeamAmountCase() 
		...
		assertTrue(world.getTeams().size()<10);
	}
	...
	public void testGetActiveProjectile() {
		...
		assertTrue(((ArrayList<Worm>)world.getWorms()).get(0).getProjectile().isActive());
	}
	...
	public void testRemoveProjectileFromWorld(){
		...
		assertTrue(world.getCurrentWorm().isTerminated());
	}
	@Test
	public void testRemoveWormFromWorld() {
		World world = new World(5.0,5.0, passable, ran);
		int x = world.getWorms().size();
		world.addWormToWorld();
		world.addWormToWorld();
		world.startGame();
		Worm worm = world.getCurrentWorm();
		world.startNextTurn();
		world.removeWormFromWorld(worm);
		assertEquals(x-1, world.getWorms().size());
	}
	@Test
	...
	public void testGetObjectsInWorld() {
		...
		assertEquals(((ArrayList<Worm>)world.getWorms()).get(0), (((ArrayList<Object>)world.getObjectsInWorld()).get(0)));
	}
	@Test
	...
	public void testStartGame_LegalCase (){
		...
		world.addWormToWorld();
		world.startGame();
		assertSame(world.getCurrentWorm(),((ArrayList<Worm>)world.getWorms()).get(0));
	}
	...
	public void testCreateFood_IllegalBoundCase()
		...
		try{
			...
			assertTrue("Invalid Coordinates",world.getFood().size()==0);
		}
	}
	@Test
	public void testCreateWorm_LegalCase() {
		World world = new World(5.0,5.0, passable, ran);
		world.createWorm(1.0, 1.0, 1.0, 0.25, "Test");
		assertEquals(((ArrayList<Worm>)world.getWorms()).get(0).getName(),("Test"));

	}
	
	@Test (expected = IllegalArgumentException.class)
	public void testCreateWorm_OutOfWorldBoundsCase() throws Exception{
		World world = new World(5.0,5.0, passable, ran);
	...
	public void testCreateWorm_IllegalRadiusCase() {
		...
		World world = new World(5.0,5.0, passable, ran);
		try{
		world.createWorm(1.0, 1.0, 0.0, -1.0, "Test");
		}catch(IllegalArgumentException exc){
			fail("Invalid Radius");
		}
	}
